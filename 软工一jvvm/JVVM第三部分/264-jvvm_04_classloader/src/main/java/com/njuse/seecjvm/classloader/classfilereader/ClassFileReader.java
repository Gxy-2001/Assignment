...
public class ClassFileReader {
    ...
    public Pair<byte[], Integer> readClassFile(String className, EntryType privilege) throws IOException, ClassNotFoundException {
        ...
        byte[] arr;
        arr = bootClasspath.readClass(realClassName);
        if (arr != null) {
            return Pair.of(arr, EntryType.BOOT_ENTRY);
        }
        arr = extClasspath.readClass(realClassName);
        if (arr != null) {
            return Pair.of(arr, EntryType.EXT_ENTRY);
        }

        arr = userClasspath.readClass(realClassName);
        if (arr != null) {
            return Pair.of(arr, EntryType.USER_ENTRY);
        }
